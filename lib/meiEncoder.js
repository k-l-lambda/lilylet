import { Clef, Accidental, OrnamentType, StemDirection, HairpinType, PedalType, } from "./types";
...
const encodePitch = (pitch, keyFifths = 0) => {
...
const buildNoteElement = (pitch, dur, dots, indent, inChord, options = {}, noteId) => {
    ...
    if (!inChord && options.slur)
        attrs += ` slur="${options.slur}"`;
...
const extractMarkOptions = (marks) => {
    ...
    for (const mark of marks) {
        // Articulations
        if ('type' in mark && ARTIC_MAP[mark.type]) {
            result.artics.push({
                type: ARTIC_MAP[mark.type],
                placement: mark.placement,
            });
        }
        // Ornaments
        const ornamentType = mark.type;
...
const noteEventToMEI = (event, indent, layerStaff, tieEnd, contextStemDir, keyFifths = 0) => {
    ...
    const slurParts = [];
    if (markOptions.slurStart)
        slurParts.push('i');
    if (markOptions.slurEnd)
        slurParts.push('t');
...
const restEventToMEI = (event, indent, keyFifths = 0) => {
    const dur = DURATIONS[event.duration.division] || "4";
...
const tupletEventToMEI = (event, indent, layerStaff, keyFifths = 0) => {
    ...
    for (const e of event.events) {
        ...
        if (e.type === 'note') {
            result += noteEventToMEI(e, currentIndent, layerStaff, false, undefined, keyFifths).xml;
        }
        else if (e.type === 'rest') {
            result += restEventToMEI(e, currentIndent, keyFifths);
        }
...
const tremoloEventToMEI = (event, indent, keyFifths = 0) => {
...
const encodeLayer = (voice, layerN, indent, initialTiePitches = [], keyFifths = 0) => {
    ...
    for (const event of voice.events) {
        ...
        switch (event.type) {
            ...
            case 'rest':
                xml += restEventToMEI(event, currentIndent, keyFifths);
                break;
            case 'tuplet':
                xml += tupletEventToMEI(event, currentIndent, voice.staff, keyFifths);
                break;
            case 'tremolo':
                xml += tremoloEventToMEI(event, currentIndent, keyFifths);
                break;
            case 'context': {
...
// Encode a staff
const encodeStaff = (voices, staffN, indent, tieState = {}, keyFifths = 0) => {
...
const generateTempoElement = (tempo, indent) => {
...
const encodeMeasure = (measure, measureN, indent, totalStaves, tieState, keyFifths = 0, partInfos = []) => {
